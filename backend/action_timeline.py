...
def analyze_video(video_path: str, output_json_path: str) -> dict:
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        audio_segments = []
        if INCLUDE_AUDIO and _has_audio_stream(video_path):
            try:
                _extract_audio(video_path, audio_path)
                audio_segments = _transcribe_audio(audio_path)
            except subprocess.CalledProcessError:
                # Some uploads report an audio stream but still fail extraction.
                # Keep analysis usable by falling back to visuals only.
                audio_segments = []
